    ...
    <div className="rounded-xl bg-card border border-border shadow-sm p-8 animate-fade-in">
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        ...
        <ProjectCard
          name="CASAMAX"
          date="Jan 2025"
          stack={["NextJS", "Typescript", "Tailwind", "NextAuth", "Keycloak", "Zustand"]}
...
function ProjectCard({
  name, date, stack, desc
}: {
  name: string, date: string, stack: string[], desc: string
}) {
  return (
    <div className="bg-muted/60 rounded-lg border border-border p-6 flex flex-col gap-2 shadow-lg hover:scale-[1.03] transition-transform animate-fade-in">
      ...
      <div className="flex flex-wrap gap-2 my-1">
        {stack.map((tech) => (
          <span key={tech} className="bg-accent px-2 py-0.5 rounded text-xs">{tech}</span>
        ))}
      </div>
